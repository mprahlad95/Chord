...
defmodule ChordRing do
  ...
  defp set_finger_tables_in_children(node_id_map, m, maxNumNodes) do
    ...
    Enum.each  node_id_map,  fn {node_id, node_pid} ->
      finger_table = get_finger_table(node_id, node_id_map, m, maxNumNodes, 0, [])
      [succ_id, succ_pid] = get_succ(node_id, node_id_map, maxNumNodes)
      GenServer.call(node_pid, {:set_finger_table, finger_table, succ_id, succ_pid})
      GenServer.call(succ_pid, {:set_prev, node_id, node_pid})
    end
  end

  defp get_finger_table(node_id, node_id_map, m, maxNumNodes, power, finger_table) do
    ...
    cond do
      ...
      true ->
        ...
        get_finger_table(node_id, node_id_map, m, maxNumNodes, power+1, finger_table)
    end
  end
...
defmodule ChordNode do
  ...
  defp closest_preceding_node(finger_table, search_for_node, curr_node_id, maxNumNodes) do
    Enum.find(finger_table, fn {x, _} -> is_contained?(curr_node_id+1, x, search_for_node-1, maxNumNodes) end)
  end

  defp is_contained?(starting, id, ending, maxNumNodes) do
