...
defmodule ChordRing do
  ...
  def main do
    ...
    {:ok, super_pid} = start_link(numNodes, maxNumNodes, m)
  ...
  def start_link(numNodes, maxNumNodes, m) do

    r = rem(maxNumNodes, numNodes)

    children = get_children_list([], 0, r, numNodes, m)
    Supervisor.start_link(children, strategy: :one_for_one)
  end

  defp get_children_list(children_list, currNode, remainder, totalNodes, m) do
    ...
    if totalNodes == 0 do
      ...
      [%{
        id: currNode,
        start: {ChordNode, :start_link, [%{:id => currNode, :node_identifier => :crypto.hash(:sha, Integer.to_string(currNode)) |> Base.encode16, :message_map => %{}, :requests_sent => 0, :m => m, :super_pid => self()}]}
      }] ++ get_children_list(children_list, nextNode, remainder - 1, totalNodes - 1, m)
    end
  end
    ...
    receive do
      {:message_found, _node_id, hops} ->
        ...
        if num_messages_found < numNodes*numRequests do
          ...
          IO.puts "Average hops for #{numNodes} nodes each making #{numRequests} requests is #{total_hops / (numRequests*numNodes)}"
          IO.puts "Removing 5 nodes"
          listen(numNodes-5, numRequests, num_messages_found, 0)
        end
    end
  end
...
defmodule ChordNode do
  ...
  def handle_cast({:stabilize, maxNumNodes}, state) do
    IO.puts "inside stabilize1 #{inspect state}"
    succ_pid = state.succ_pid
    state = if succ_pid != nil do
      succesors_pred_pid = GenServer.call(succ_pid, {:get_pred})
      #IO.puts "inside stabilize2 #{inspect succesors_pred_pid}"
      succ_id = GenServer.call(succ_pid, {:get_id})
      #IO.puts "inside stabilize4 #{inspect succ_id}"
      #IO.puts "inside stabilize3 #{inspect self()}"
      if succesors_pred_pid != self() do
        succesors_pred_id = GenServer.call(succesors_pred_pid, {:get_id})
        node_id = state.id

        cond do
          belongs(succesors_pred_id, node_id+1, succ_id-1, maxNumNodes) and succesors_pred_pid != self()->
            GenServer.cast(succesors_pred_pid, {:notify, self(), maxNumNodes})
            Map.put(state, :succ_pid, succesors_pred_pid)
          true ->
            IO.puts "succ_pred same as self"
            state
        end
      end
    end
    ...
    IO.puts "returning from stabilize"
    {:noreply, state}
  end

  def handle_cast({:fix_fingers, maxNumNodes}, state) do
    next = state.next + 1
    next = if(next > state.m) do
      1
    else
      next
    end
    [node_id, _hops] = GenServer.call(self(), {:find_successor, state.id + :math.pow(2, next-1), 0, 1, maxNumNodes})
    finger_table = state.finger_table
    finger_table = Enum.slice(finger_table, 0..next-1) ++ [{node_id, self()}] ++ Enum.slice(finger_table, next+1..-1)
    state = Map.put(state, :next, next)
    state = Map.put(state, :finger_table, finger_table)
    IO.puts "fix_fingers: node_id: #{state.id}; finger_table: #{inspect state.finger_table}"
    {:noreply, state}
  end

  # // tentative_pred_pid thinks it might be our predecessor
  def handle_cast({:notify, tentative_pred_pid, maxNumNodes}, state) do
    pred_pid = state.pred_pid
    pred_id = if pred_pid == self(), do: state.id, else: GenServer.call(pred_pid, {:get_id})
    tentative_pred_id = GenServer.call(tentative_pred_pid, {:get_id})
    state = if(pred_pid == nil or belongs(tentative_pred_id, pred_id+1, state.id-1, maxNumNodes)) do
      Map.put(state, pred_pid, tentative_pred_pid)
    else
      state
    end
    # IO.inspect state

    #IO.puts "node_id: #{state.id}; succ_id: #{inspect state.succ_pid}; pred_id: #{inspect state.pred_pid}"
    {:noreply, state}
  end
